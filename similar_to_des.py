def rotate_left(x, shift, size=32):
    return ((x << shift) & (2**size - 1)) | (x >> (size - shift))

S_BOX = [
    
]

def s_box(value):
    return S_BOX[value]

def round_function(R, K):
    R0 = (R >> 24) & 0xFF
    R1 = (R >> 16) & 0xFF
    R2 = (R >> 8) & 0xFF
    R3 = R & 0xFF
    
    R0 = s_box(R0)
    R1 = s_box(R1)
    R2 = s_box(R2)
    R3 = s_box(R3)
    
    combined = (R0 << 24) | (R1 << 16) | (R2 << 8) | R3
    
    permuted = rotate_left(combined, 11, 32)
    
    return permuted ^ K

def generate_subkeys(key):
    K_L = (key >> 32) & 0xFFFFFFFF
    K_R = key & 0xFFFFFFFF
    
    subkeys = []
    for i in range(8):
        combined = (K_L << 32) | K_R
        subkeys.append(combined & 0xFFFFFFFF)
        
        K_L = rotate_left(K_L, 3, 32)
        K_R = rotate_left(K_R, 5, 32)
    
    return subkeys

def feistel_encrypt(plaintext, key):
    subkeys = generate_subkeys(key)
    
    L = (plaintext >> 16) & 0xFFFF
    R = plaintext & 0xFFFF
    
    for i in range(8):
        L, R = R, L ^ round_function(R, subkeys[i])
    
    ciphertext = (L << 16) | R
    return ciphertext


def main():
    key = 0x1122334455667788
    plaintext = 0x0123456789ABCDEF
    
    ciphertext = feistel_encrypt(plaintext, key)
    print(f"Plaintext: 0x{plaintext:016X}")
    print(f"Ciphertext: 0x{ciphertext:016X}")

if __name__ == "__main__":
    main()
